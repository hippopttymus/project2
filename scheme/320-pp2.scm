...
(display "----------")
(newline)
(display "Number of Customer(s) whose postal codes begin with 547: ")
(display (extract (postcode_begins_with "547") billdata_path))

(display "----------")
(newline)
(display "Number of Customer(s) who Ordered this month : ")
(length (extract ordered_this_month billdata_path))

(newline)
(display "----------")
(newline)
(display "Number of Customer(s) from whitewater who ordered this month :")
(for-each (map (lambda (customer) (string-append(cdr (assoc "First Name" customer)))) (extract (postcode_begins_with "53191") billdata_path)))
